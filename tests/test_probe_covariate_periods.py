...
def _patch_client(rows: list[list[Any]]) -> Any:
    """Patch Dhis2Credentials.get_client to return a stub whose
    ``get_analytics_data`` returns ``{"rows": rows}``."""
    fake_client = MagicMock()
    fake_client.get_analytics_data.return_value = {"rows": rows}
    return patch.object(Dhis2Credentials, "get_client", return_value=fake_client)


def test_probe_picks_max_period_per_data_element() -> None:
...
def test_probe_keeps_max_when_rows_arrive_out_of_order() -> None:
...
def test_probe_skips_rows_shorter_than_four_columns() -> None:
...
def test_probe_returns_empty_dict_when_no_rows() -> None:
...
def test_probe_uses_period_key_not_lexicographic_ordering() -> None:
    ...
    is "someone replaced ``period_key`` with plain string compare". Adding
    a weekly fixture here would couple the test to internals; the existing
    coverage of ``period_key`` itself in ``dhis2-client`` is enough."""
    rows = [
        _row("POP1", "202612"),
